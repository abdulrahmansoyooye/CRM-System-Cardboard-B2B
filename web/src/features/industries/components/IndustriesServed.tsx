...
import { TIndustry } from "@/types";

const STATIC_ICONS = [ShoppingCart, Truck, Zap, Cpu, Microscope, Leaf];

export function IndustriesServed({ industries = [] }: { industries?: TIndustry[] }) {
  return (
        ...
        {industries.length === 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5 border border-white/5">
            {Array(6).fill(0).map((_, i) => (
              <div key={i} className="h-64 bg-white/2 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5 border border-white/5">
            {industries.map((ind, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <Link href={`/industries/${ind.slug}`} className="relative z-10 flex flex-col h-full">
                    ...
                    <p className="text-white/40 group-hover:text-[#0A0F1A]/70 text-sm font-bold tracking-tight leading-relaxed transition-colors duration-500 max-w-60">
                      {ind.overview || "Specialized corrugated packaging solutions for this sector."}
                    </p>
