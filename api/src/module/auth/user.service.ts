...
export const loginUser = async (payload: LoginPayloadDTO) => {
    ...
    if (!isMatched) throw new AppError('Incorrect Password', 401);

    if (!user.isActive) throw new AppError('This user is inactive!', 403);
...
export const UserService = {
    createUser, loginUser, refreshToken, getUsers, getUserById, updateUser, deactivateUser, logoutUser
}
